...
module.exports.action = function () {
  ...
  while (Math.abs(initBounds.x - targetBounds.x) > 5) {
    Safari.windows[0].bounds = initBounds = {
      ...initBounds,
      x: initBounds.x - Math.sign(initBounds.x - targetBounds.x) * Math.sqrt(Math.abs(initBounds.x - targetBounds.x)),
      y: initBounds.y - Math.sign(initBounds.y - targetBounds.y) * Math.sqrt(Math.abs(initBounds.y - targetBounds.y)),
    }
  }
  ...
  while (Math.abs(initBounds.width - targetBounds.width) > 5) {
    Safari.windows[0].bounds = initBounds ={
      ...initBounds,
      width: initBounds.width - Math.sign(initBounds.width - targetBounds.width) * Math.pow(Math.abs(initBounds.width - targetBounds.width), 0.7),
      height: initBounds.height - Math.sign(initBounds.height - targetBounds.height) * Math.pow(Math.abs(initBounds.height - targetBounds.height), 0.7),
    }
  }
  delay(1)
